<?php
	...
	//Paginador
	$sql_registe = mysqli_query($conection,"SELECT COUNT(*) as total_registro FROM serie WHERE status != 10 ");
	...
	$query = mysqli_query($conection,"SELECT * FROM serie WHERE status != 10 ORDER BY idserie ASC LIMIT $desde,$por_pagina");
...
<body>
	...
	<section id="container">
		<h1> <i class="fas fa-file-alt"></i></i> Series</h1>
		...
		<?php
			if($result > 0){
				...
				while ($data = mysqli_fetch_array($query)) {
					if($data["status"] == 1){
						$status = '<span title="Desactivar Serie" class="pagada activeUser" estado="'.$data["status"].'" onclick="modalActiveDocumento('.$data["idserie"].');">Activo</span>';
					}else{
						$status = '<span title="Activar Serie" class="anulada activeUser" estado="'.$data["status"].'" onclick="modalActiveDocumento('.$data["idserie"].');">Inactivo</span>';
					}
			?>
				<tr id="item_<?php echo $data["idserie"]; ?>" >
					<td><?php echo $data["idserie"]; ?></td>
