<?php
	...
	//Paginador
	$sql_registe = mysqli_query($conection,"SELECT COUNT(*) as total_registro FROM tipo_pago
															WHERE ( id_tipopago LIKE '%$busqueda%' OR
																tipo_pago LIKE '%$busqueda%'
														   	) AND estatus != 10 ORDER BY tipo_pago ASC ");
	...
	$query = mysqli_query($conection,"SELECT * FROM tipo_pago
												WHERE
													( id_tipopago LIKE '%$busqueda%' OR
													  tipo_pago LIKE '%$busqueda%' )
													AND
													estatus != 10 ORDER BY tipo_pago ASC LIMIT $desde,$por_pagina
									");
...
<body>
	...
	<section id="container">
		<h1> <i class="fas fa-file-alt"></i> Formas de pago</h1>
		...
		<?php
			if($result > 0){
				...
				while ($data = mysqli_fetch_array($query)) {
					if($data["estatus"] == 1){
						$estatus = '<span title="Desactivar Marca" class="pagada activeUser" estado="'.$data["estatus"].'" onclick="modalActiveTipoPago('.$data["id_tipopago"].');">Activo</span>';
					}else{
						$estatus = '<span title="Activar Marca" class="anulada activeUser" estado="'.$data["estatus"].'" onclick="modalActiveTipoPago('.$data["id_tipopago"].');">Inactivo</span>';
					}
