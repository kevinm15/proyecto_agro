<?php
	...
	//Paginador
	$sql_registe = mysqli_query($conection,"SELECT COUNT(*) as total_registro FROM ubicacion
															WHERE ( id_ubicacion LIKE '%$busqueda%' OR
																ubicacion LIKE '%$busqueda%' OR
																descripcion LIKE '%$busqueda%'
														   	) AND status != 10 ORDER BY ubicacion ASC ");
	...
	$query = mysqli_query($conection,"SELECT * FROM ubicacion
												WHERE
													( id_ubicacion LIKE '%$busqueda%' OR
													  ubicacion LIKE '%$busqueda%' OR
													  descripcion LIKE '%$busqueda%' )
													AND
													status != 10 ORDER BY ubicacion ASC LIMIT $desde,$por_pagina
		");
